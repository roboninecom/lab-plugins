...
type ViewState = 'idle' | 'recording' | 'labeling'
...
export function PluginRoot({ context }: Props) {
  ...
  const { Button, CameraView, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } = context.ui
  ...
  const handleStart = useCallback(() => {
    ...
    const loop = async () => {
      while (running) {
        const positions = await servoRef.current.readJointPositions()
        ...
        if (positions) {
          ...
          framesRef.current.push({ seq: seq++, ts: Date.now(), joints, image, imageWidth, imageHeight })
          setFrameCount(seq)
        }
